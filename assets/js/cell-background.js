...
const can = document.getElementById('main-canvas');
...
tiles[toIndex(size-1,0)] = new Creature(new Rgb(0, 0, 255));
tiles[toIndex(Math.round(size / 2),0)] = new Creature(new Rgb(0, 255, 0));
tiles[toIndex(size-1,size-1)] = new Creature(new Rgb(255, 255, 255));
tiles[toIndex(0,size-1)] = new Creature(new Rgb(0, 0, 0));
let lastUpdate = Date.now();
update();
function update () {
    if (window.innerWidth > window.innerHeight) {
        if (can.width != window.innerWidth || can.height != window.innerWidth) {
            can.width = window.innerWidth;
            can.height = window.innerWidth;
        }
    } else {
        if (can.width != window.innerHeight || can.height != window.innerHeight) {
    ...
    for (let i = 0; i < Math.min(Math.floor(ticksExpected), 300); i++) {
        fixedUpdate();
    }
...
function draw () {
    ...
    for (let i = 0; i < size*size; i++) {
        ...
        if (tiles[i]) {
            ...
            ctx.fillRect(pos.x*(can.width/size),pos.y*(can.width/size),can.width/size*1.02,can.width/size*1.02);
        }
    }
}
